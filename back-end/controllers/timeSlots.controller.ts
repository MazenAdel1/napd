...
export const getTimeSlotsByDate = asyncWrapper(
...
export const addTimeSlot = asyncWrapper(
...
export const addMultipleTimeSlots = asyncWrapper(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    while (currentMinutes + durationInMinutes <= endTotalMinutes) {
      ...
      const slotStartTime = new Date(
        new Date(date as string).setHours(slotStartHour, slotStartMinute, 0, 0),
      );
      const slotEndTime = new Date(
        new Date(date as string).setHours(slotEndHour, slotEndMinute, 0, 0),
      );
...
export const deleteTimeSlot = asyncWrapper(
  async (req: Request, res: Response) => {
    const { id } = req.query;
...
export const bookTimeSlot = asyncWrapper(
  async (req: Request, res: Response) => {
    const { id } = req.query;
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (timeSlot?.startTime && timeSlot?.startTime < new Date()) {
